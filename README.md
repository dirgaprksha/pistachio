# Pistachio

A utility library with a clear separation of concerns. Pistachio is a collection of utility packages designed two distinct purposes:

1. **Core package** - Pure utilities for common programming tasks
2. **Adapter packages** - Hooks/composables for reactive programming tasks

## Packages

| Package                                | Version                                                 | Description       |
| -------------------------------------- | ------------------------------------------------------- | ----------------- |
| [@pistachiojs/core](./packages/core)   | ![npm](https://img.shields.io/npm/v/@pistachiojs/core)  | Utility functions |
| [@pistachiojs/react](./packages/react) | ![npm](https://img.shields.io/npm/v/@pistachiojs/react) | React hooks       |
| [@pistachiojs/vue](./packages/vue)     | ![npm](https://img.shields.io/npm/v/@pistachiojs/vue)   | Vue composables   |

## License

MIT © [Dirga Prakesha](https://github.com/dirgaprksha)
